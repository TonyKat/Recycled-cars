...
def main_menu():
    try:
        ...
        print('\nГлавное меню:'
              ...
              '\n2. Получить список не утилизированных авто в указанную дату, нажмите - "2".'
        ...
        if number == '1':
            ...
            date_auto = input_date('Введите дату ')
            cur.execute('SELECT vin, pts, begin_date, end_date FROM public."Recycled_cars" '
                        'WHERE %s between begin_date and end_date;', (date_auto,))
            rows = cur.fetchall()
            print_list(rows)
            if len(rows) != 0:
                input_data("^[01]{1}$", 'Для просмотра скана PTS - нажмите "1",'
                                        ' а затем введите номер автомобиля в выведенном списке: ', number)
                while True:
                    try:
                        regex = str("^[0-9]" + "{0," + str(len(str(rows.index(rows[-1])))) + "}" + "$")
