    ...
    try:
        ...
        if number == '1':
            ...
            cur.execute('SELECT vin, pts, begin_date, end_date FROM public."Recycled_cars" '
                        'WHERE (begin_date <= %s AND %s < end_date) or '
                        '(begin_date <= %s AND %s < end_date) or '
                        '(begin_date between %s and %s)',
                        (begin_interval, begin_interval, end_interval, end_interval, begin_interval, end_interval))
            rows = cur.fetchall()
            print('\nВыводится список автомобилей бывших в строю в указанный период.\n'
                  '-----------------------------------------------------------------\n'
                  '(В день утилизации автомобиль не считается в строю.)')
            print_list(rows)
            if len(rows) != 0:
